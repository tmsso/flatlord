...
- **Real cross-repo PAT path**: the full pipeline run above used the admin's own broad `gh` credentials, not the narrow fine-grained `BACKUP_REPO_TOKEN` the scheduled workflow actually uses. First real `workflow_dispatch` run with the production secret is the true end-to-end confirmation — see status note below.
