import { pgTable, uuid, numeric, date, text, timestamp, index } from "drizzle-orm/pg-core";
...
export const meterReadings = pgTable(
  ...
  (table) => [
    ...
    index("meter_readings_tenancy_id_idx").on(table.tenancyId),
    index("meter_readings_entered_by_idx").on(table.enteredBy),
    index("meter_readings_confirmed_by_idx").on(table.confirmedBy),
  ],
);
