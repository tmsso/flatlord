...
Companion to `CLAUDE.md` (domain rules — read first; note the privacy rule §0) and `IDEAS.md` (loose backlog). Phases are sequential; each ends with a demoable milestone. Suggested cadence: one phase per Claude Code batch, plan-mode review of schema/API before implementing.

## Phase 0 — Foundation

- Repo scaffold: Next.js App Router + TypeScript strict, Tailwind, shadcn/ui, next-intl (hu/en), dark mode, base layouts (admin shell + tenant shell), design tokens per CLAUDE.md §5. **First commit includes `.gitignore` with `/private`.**
- Decide ORM approach (Drizzle vs supabase-js + generated types) — one-pager, then commit to it.
- Supabase project: Auth (magic link + password), initial migrations, RLS scaffolding, Storage buckets (`contracts`, `attachments`, `meter-photos`, `inventory`).
- Core schema (effective-dated, multi-property-ready):
  - `users`, `profiles` (role, locale)
...
**Accept:** both roles log in and see role-appropriate shells in HU and EN; RLS test proves tenancy isolation; repo contains zero real-world data.

## Phase 1 — Billing core, meter flow, backup (MVP, replaces the Google Sheet)

- Charge model: `charge_types` (fixed / metered / one-off), `charge_schedules` (`valid_from`/`valid_to`), `adjustments` (± amount, reason, target month).
- Meters & readings: `meters`, `meter_readings` (entered/ocr/confirmed columns per CLAUDE.md §3.4). **Phone-first tenant flow**: browser camera capture per meter (capture attribute; evaluate getUserMedia live preview) + file-upload fallback, typed value with ≥previous validation; admin verification; meter-replacement flow.
...
- Admin: property/tenancy/person CRUD, rate management with history, statement review/issue, payment recording.
