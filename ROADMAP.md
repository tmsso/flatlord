...
**Status (2026-07-29): in progress, not yet accepted — one item left.** Done: charge model + billing engine, phone-first meter-reading flow, statement lifecycle + payments UI, the Google Sheet importer with golden tests, Backup v1 in full, **admin property/tenancy/person CRUD + the field-requirement engine's UI** (`properties`/`tenancies`/`persons` nav all live, field_requirements-driven "required because…" banner working end-to-end), **tenant portal's key-rental-data + own/co-occupant details** (`/home/settings` "My tenancy" page), and **charts v1** (per-meter consumption + monthly cost, both built from `statement_line_items`, never a hardcoded charge-type set). All three verified via real headless-Playwright click-throughs against the dev/staging Supabase project, not just reviewed — see commits `90d0931`, `0e0824f`, `f5291ca`. Two new migrations landed with that work: `0012` (persons CRUD columns + owner write-RLS on properties/tenancies/persons/tenancy_occupants/property_ownership/audit_log) and `0013` (tenant read access to `properties`, which had none before). **Both are applied to the dev/staging project only — neither has been applied to `flatlord-prod` yet.** Apply-to-prod is a real infra decision (needs explicit go-ahead, not autonomous) — do this before/alongside accepting the phase, not as an afterthought.

Remaining before this phase can be accepted: **amount-due delivery** (bilingual Resend email + `wa.me` button) — blocked, not just unstarted: no `RESEND_API_KEY`/sender configured in Vercel prod at all, and Resend's sandbox sender only delivers to the account owner's own address, so a real sending domain needs to be decided (buy/verify one, or accept owner-only test sends for now) before this can be built end-to-end. That decision is the admin's to make.

**Suggested next session starting point:** resolve the Resend domain question (see above), then build amount-due delivery — it's the last item blocking Phase 1 acceptance. Once that's in, do the prod migration/deploy pass (0012 + 0013 + all three feature commits) before formally accepting the phase.

**Priority for whichever phase picks up next (2026-07-29):** the dev/staging Supabase project (`kqrzqbocruwafvuodatb`) received a pause notice — free-tier projects auto-pause after a period of API inactivity. Build a **keep-alive mechanism** before this becomes a recurring blocker (it would break every future dev-session verification, not just be an inconvenience): a small scheduled job (GitHub Action cron, same pattern as the existing nightly prod backup Action) that runs a trivial query against the dev project on a schedule short enough to reset Supabase's inactivity timer. This is explicitly higher priority than it looks on the phase list — the admin flagged it directly, put it first.
