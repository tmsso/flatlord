...
**Status (2026-08-15):** first 3 of this phase's 7 bullets shipped and live in prod (migrations `0015`–`0016`) — contract module, contract intake parsing, deposit ledger. The 7th bullet ("tenant sees contracts, inventory, deposit status") is now *partially* satisfied — contracts and deposit status ship with items 1–3, inventory is untouched. **Phase 2 is not accepted** — generic attachments, document template generator, and the inventory module (items 4–6) are still open, and this Accept clause needs all of them.
