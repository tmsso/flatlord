...
**Status (2026-08-03): in progress, not yet accepted.** Done and prod-verified: charge model + billing engine, phone-first meter-reading flow, statement lifecycle + payments UI, the Google Sheet importer with golden tests, Backup v1 in full, admin property/tenancy/person CRUD + the field-requirement engine's UI, tenant portal's key-rental-data + own/co-occupant details, charts v1, a dev/staging Supabase keep-alive Action (`.github/workflows/dev-keepalive.yml`, daily REST ping), and a reusable prod-migration mechanism (`.github/workflows/migrate-prod.yml`, `workflow_dispatch`-only) — migrations `0012`–`0014` are now all applied to `flatlord-prod`, not just dev. **Amount-due delivery is built and deployed** (bilingual Resend email + `wa.me` deep-link button, per-statement, built from the tenant's own stored locale, gated to statements with an actual outstanding balance) but **email sending is unproven end-to-end**: no `RESEND_API_KEY` exists anywhere yet (not `.env.local`, not Vercel prod/preview) — the admin is adding one after this session. The `wa.me` path has no such dependency and is fully live-verified.

Remaining before this phase can be accepted:

- **Blocked on the admin:** `RESEND_API_KEY`/`RESEND_FROM_EMAIL` need to be set (Vercel prod + preview + `.env.local`) before email sending can be proven — admin said they'd add these after the 2026-08-03 session. Retiring the Google Sheet to a read-only archive is also the admin's own call, a deliberate action outside the code, not something a session should do on its own initiative.
- **Next up once unblocked:** send one real amount-due email to confirm delivery actually works end-to-end, then walk one real statement through the full monthly cycle (photo → verification → statement → email/WhatsApp → payment) to satisfy the Accept clause below.
