...
**Status (2026-07-23): in progress, not yet accepted.** Done: charge model + billing engine (fixed/metered/tracked-only, effective-dated schedules, adjustments), phone-first meter-reading flow (tenant capture + admin verification, real end-to-end click-through verified), statement lifecycle (draft→issued→partially_paid/paid/overdue) + payments UI for both roles, the Google Sheet importer with golden tests, and Backup v1 in full (in-app export + nightly GitHub Action, restore genuinely tested with exact row-count parity — not just designed). Remaining before this phase can be accepted: amount-due delivery (bilingual Resend email + `wa.me` button — not started), admin property/tenancy/person CRUD + the field-requirement engine's UI (schema exists in `field_requirements`/`field_policies`, zero UI consumes it yet — admin nav "properties"/"tenancies" still dead-linked, `/dashboard` is a placeholder), tenant portal's key-rental-data + own/co-occupant details (tenant home currently shows amount-due only, not the rest of the read-only scope this phase calls for), and charts v1 (recharts not yet wired in). Don't start Phase 2 before these land — see the handoff note below on sequencing.

**Suggested next session starting point:** admin property/tenancy/person CRUD — it's the largest of the four remaining items, and the field-requirement engine (schema already built) and the dead `properties`/`tenancies` nav links both depend on it, so it unblocks the most.
