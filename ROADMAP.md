...
- ~~**Contract intake parsing**: on upload, extract text (digital PDF directly; scans via OCR — OpenRouter multimodal acceptable here already) → propose prefilled key terms → **admin reviews field-by-field, nothing auto-committed**.~~ **Shipped, scoped down** (PR #15): digital-PDF text-layer extraction + regex/heuristic term proposal (no LLM) works end-to-end, admin accepts/edits/ignores per field. **Scanned PDFs are not OCR'd** — they get a clear "OCR not configured" fallback for manual entry, by explicit admin decision, not a partial implementation of the original bullet. `src/server/contracts/parse-contract.ts` is the seam a future session would extend with an OpenRouter multimodal call. `OPENROUTER_API_KEY` was added to `.env.local` (2026-08-15, local dev only — not yet in Vercel), model choice discussed (a free-tier Gemini Flash vision model is the recommendation — see memory), but wiring the actual call is deliberately deferred. **Still the one unclosed Accept-clause gap**: "existing signed contract uploaded → parsed → confirmed terms populate the tenancy" has only been proven against a synthetic digital-text PDF on dev, never a real scanned contract.
...
**Accept:** existing signed contract uploaded → parsed → confirmed terms populate the tenancy (⚠️ proven only on a synthetic digital-text PDF, not a real scanned one — see contract intake parsing above); conditional-ownership item representable (✅ end-to-end verified against dev); declaration PDF generated in both languages (✅ diacritics round-trip test); manual inventory reconfirmation round-trips (✅ end-to-end verified against dev, including the discrepancy → needs-review-queue path).
