...
Remaining before this phase can be accepted: (1) prove real email delivery once `RESEND_API_KEY` is configured; (2) execute one full real monthly cycle end-to-end per the Accept clause below, including sending an actual amount-due notice and recording the resulting payment; (3) the admin manually retires the Google Sheet to a read-only archive — that's a deliberate action outside the code, not something to do automatically.

**Suggested next session starting point:** confirm `RESEND_API_KEY`/`RESEND_FROM_EMAIL` are set (Vercel prod + preview + `.env.local`), send one real amount-due email, then walk a real statement through the full cycle to close out the Accept clause.
